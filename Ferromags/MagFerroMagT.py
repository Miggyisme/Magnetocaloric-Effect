...
# Leitura dos dados estritamente do arquivo input.dat
...
# Configuracao do campo magnetico
B_inicial = 0
B_final = 50
N = 5000
...
dB = (B_final - B_inicial) / N
campos = [B_inicial + i * dB for i in range(N + 1)]
...
def salvar(nome_arquivo, campos, resultados):
    with open(nome_arquivo, "w") as f:
        for campo, res in zip(campos, resultados):
            f.write(f"{campo:.6f}\t{res:.6e}\n")
salvar(nome_saida, campos, dM_dT_results)

plt.scatter(campos, dM_dT_results)
